...
app.get("/health", (_req, res) => {
  console.log("[GAME][HEALTH] Health check called");
...
server.on("upgrade", (req, socket, head) => {
  console.log("[GAME][CONNECT] Incoming WebSocket connection:", req.url);
  wss.handleUpgrade(req, socket, head, (ws) => {
    console.log("[GAME][ACCEPT] WebSocket accepted");
    ws.send("✅ Connected to Game service (port 7002)");

    ws.on("message", (msg) => {
      console.log("[GAME][RECV]", msg.toString());
      ws.send(`Server echo: ${msg}`);
    });

    ws.on("close", () => {
      console.log("[GAME][CLOSE] Client disconnected");
    });
  });
});
