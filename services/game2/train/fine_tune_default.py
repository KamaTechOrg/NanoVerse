...
from .replay import sample_replay_events_pooled
...
def fine_tune_default(start_ts: float, end_ts: float,
                      H=64, W=64, lr=8e-4, epochs=6, batch_size=128,
                      init_from_previous=True, min_samples=500,
                      replay_ratio=0.2, history_days=7):
    users_root = Path("data/users")
    today_events: List[dict] = []
    for udir in users_root.iterdir():
        if not udir.is_dir(): continue
        f = udir/"actions.jsonl"
        if not f.exists(): continue
        today_events.extend(list(filter_by_time(iter_events_jsonl(f), start_ts, end_ts)))
    if len(today_events) < min_samples:
        print(f"[default] skipped — only {len(today_events)} samples (<{min_samples})")
        return False

    replay_events = sample_replay_events_pooled(users_root, start_ts, history_days=history_days)
    k = min(int(len(today_events)*replay_ratio), len(replay_events))
    if k>0:
        import random
        today_events = today_events + random.sample(replay_events, k)

    ds = SeqDatasetFromEvents(today_events, H=H, W=W)
    ...
    if init_from_previous and default_path.exists():
    ...
    for ep in range(1, epochs+1):
        ...
        print(f"[default] ep {ep} tr={trL/max(1,trN):.4f} va={vaL/max(1,vaN):.4f}")

    from services.game2.models.bot_gru import GRUPolicy as Build
    safe_save_state_dict(model, default_path, build_model_fn=lambda: Build(), H=H, W=W, keep_backup=True)
    print(f"[default] ✅ saved -> {default_path}")
    return True
