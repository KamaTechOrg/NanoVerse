...
const VoxelGrid: React.FC = () => {
  ...
  // ✅ GRID RENDER — NEW VERSION with COLORED ICONS + SMOOTH MOVE
  const renderGrid = () => {
    if (!gameState) return null;

    const [chunkX, chunkY] = (gameState?.chunk_id || "0,0").split(",").map(Number)

    const cellW = 100 / gameState.w;
    const cellH = 100 / gameState.h;
    ...
    // ✅ Render players as icons in the color of the player
    const playerDivs = players.map((p) => {
      ...
      return (
        ...
        >
          <div
            style={{
              width: "160%",         // ✅ אייקון גדול
              height: "160%",        // ✅ אייקון גדול
              borderRadius: "10px",
      ...
      <div className="container mx-auto px-4 pt-4 sm:pt-8">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-4 sm:mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-yellow-500/20 to-orange-500/20 text-yellow-300
                     hover:from-yellow-500/30 hover:to-orange-500/30 transition-all duration-200 transform hover:scale-105"
          >
            <Trophy size={18} />
